...
import { Search, X, TrendingUp, Loader2, Plus, Star, Check } from "lucide-react";
...
const STORAGE_KEY       = "ybsg-watchlist";
const MARKET_KEY        = "ybsg-market-map";
const DEFAULT_WATCHLIST = ["0050", "00981A", "00988A", "00994A", "00935"];
...
export default function YBSGTab() {
    ...
    <div className="space-y-6 py-2">
      ...
      <div>
        <h1 className="text-2xl font-bold flex items-center gap-2" style={{ color: "var(--fg)" }}>
          <TrendingUp className="h-6 w-6 text-blue-500" /> YBSG 績效研究
        </h1>
        ...
        {showWLInput && (
          <div className="mt-3 flex items-center gap-2 border-t pt-3 flex-wrap" style={{ borderColor: "var(--border)" }}>
            ...
            <input
              ...
              onKeyDown={e => {
                if (e.key === "Enter") { e.preventDefault(); addToWatchlist(); }
                if (e.key === "Escape") { setShowWLInput(false); setWlInput(""); }
              }}
            ...
            <button
              ...
              disabled={!wlInput.trim() || wlInputLoading}
            ...
            <button
              onClick={() => { setShowWLInput(false); setWlInput(""); setWlError(""); }}
      ...
      <div className="rounded-2xl px-5 py-4 space-y-4" style={cardStyle}>
        ...
        <div className="flex flex-wrap gap-3 items-end">
          {[{ label: "起始日", val: startDate, set: setStartDate }, { label: "結束日", val: endDate, set: setEndDate }].map(({ label, val, set }) => (
            <div key={label}>
          ...
          <button
            onClick={() => fetchData(codes, startDate, endDate)}
            disabled={isLoading || !codes.length}
            className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-5 py-2 text-sm font-semibold text-white hover:bg-blue-700 disabled:opacity-50 transition-colors cursor-pointer"
          >
